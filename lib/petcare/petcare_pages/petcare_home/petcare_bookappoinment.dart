...
class PetCareBookAppoinment extends StatefulWidget {
  const PetCareBookAppoinment({Key? key}) : super(key: key);

  @override
  State<PetCareBookAppoinment> createState() => _PetCareBookAppoinmentState();
}

class _PetCareBookAppoinmentState extends State<PetCareBookAppoinment> {
  dynamic size;
  double height = 0.00;
  double width = 0.00;
  ...
  void handleBookAppointment() {
    ...
    for (int i = 0; i < myAppointments.length; i++) {
      print("${myAppointments[i].date}, ${myAppointments[i].time}");
    ...
    addAppointment(whichPet);
  }

  Future<void> addAppointment(String pet) async {
    var user = FirebaseAuth.instance.currentUser;
    String documentName = user?.uid ?? '';
    String collectionName = 'appointments';
